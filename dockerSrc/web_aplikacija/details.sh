...
uredjaj=$(su - postgres -c "psql rnms -c \"copy (select uredjaji.ip,uredjaji.hostname,uredjaji.systemname,uredjaji.snmp,deviceprofile.proizvodjac,deviceprofile.model,uredjaji.status,deviceprofile.kategorija,uredjaji.netflow,uredjaji.id from uredjaji left join deviceprofile on (uredjaji.tipuredjaja = deviceprofile.oid) where uredjaji.id="$id" ORDER BY deviceprofile.proizvodjac) to STDOUT WITH CSV HEADER;\"" | tail -n +2 | sed 's/\"//g')
sucelja=$(su - postgres -c "psql rnms -c \"copy (select sucelja.id,uredjaji.ip,sucelja.ifname,sucelja.ifalias,sucelja.ifphysaddress,sucelja.iftype,sucelja.status,sucelja.nodeid,sucelja.ip_adresa from sucelja inner join uredjaji on (nodeid = uredjaji.id) where sucelja.nodeid="$id" ORDER BY uredjaji.ip,sucelja.ifname) to STDOUT WITH CSV HEADER;\"" | tail -n +2)
IFS=$'\n'
...
if [ "$netflow" == "da" ]; then
  ...
  IFS=$'\n';
  echo "<table>"
  for line in $(nfdump -R /RNMS/netflow/${id} -s if -o csv | head -n-4); do
    ifIndex=$(echo $line | awk -F , '{print $5}');
    ifName=$(su - postgres -c "psql rnms -c \"copy (select ifname from sucelja where ifindex='$ifIndex') TO STDOUT;\"");
    if [ ! -z $ifName ]; then
      echo "<tr>"
